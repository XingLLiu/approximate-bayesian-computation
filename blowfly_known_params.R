...
resultsprefix <- "results/blowfly/known_params/"
plotprefix <- "plots/blowfly/known_params/"
...
# save(y, file = paste0(resultsprefix, "simulated_blowfly.rda"))
load(file = paste0(resultsprefix, "simulated_blowfly.rda"))
...
rej_out <- wsmc_continue(rej_out,
                          maxsimulation = 4 * 10^6,
                          savefile = paste0(resultsprefix, "rej_out.RData")
                        )
...
wabc_out <- wsmc_continue(wabc_out,
                          maxsimulation = 4 * 10^6,
                          savefile = paste0(resultsprefix, "wabc_out.RData")
                        )
...
klabc_out <- wsmc_continue(klabc_out,
                          maxsimulation = 4 * 10^6,
                          savefile = paste0(resultsprefix, "klabc_out.RData")
                        )
...
obs_df <- data.frame(time = blowfly[, 2], y = y, lab = "real")
